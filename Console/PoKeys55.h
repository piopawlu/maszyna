...
{ // komunikacja z PoKeys bez okreœlania przeznaczenia pinów
    ...
    unsigned char OutputBuffer[65]; // Allocate a memory buffer equal to our
    // endpoint size + 1
    unsigned char InputBuffer[65]; // Allocate a memory buffer equal to our
    // endpoint size + 1
    ...
    bool bNoError; // zerowany po przepe³nieniu licznika powtórzeñ, ustawiany po
    // udanej operacji
  public:
