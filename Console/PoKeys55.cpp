...
// HID\Vid_1dc3&Pid_1001&Rev_1000&MI_01 - MI_01 to jest interfejs komunikacyjny
// (00-joystick,
// 02-klawiatura)
...
// GUID
// InterfaceClassGuid={0x4d1e55b2,0xf16f,0x11cf,0x88,0xcb,0x00,0x11,0x11,0x00,0x00,0x30};
...
{ // Ra: to jest do wyczyszcznia z niepotrzebnych
    // zmiennych i komunikatów
    Close();
    ...
    // First populate a list of plugged in devices (by specifying
    // "DIGCF_PRESENT"), which are of the
    // specified class GUID.
    DeviceInfoTable =
        SetupDiGetClassDevs(&InterfaceClassGuid, NULL, NULL, DIGCF_PRESENT | DIGCF_DEVICEINTERFACE);
    // Now look through the list we just populated.  We are trying to see if any
    // of them match our
    // device.
    while (true)
    {
        ...
        // Now retrieve the hardware ID from the registry. The hardware ID contains
        // the VID and PID,
        // which we will then
        ...
        // PropertyValueBuffer
        // gets filled with a
        // REG_MULTI_SZ (array of null terminated strings). To find a device,we only
        // care about the
        // very first string in the
        // buffer,which will be the "device ID". The device ID is a string which
        // contains the VID
        // and PID,in the example
        // format "Vid_04d8&Pid_003f".
        SetupDiGetDeviceRegistryProperty(DeviceInfoTable, &DevInfoData, SPDRP_HARDWAREID,
                                         &dwRegType, PropertyValueBuffer, dwRegSize, NULL);
        // Now check if the first string in the hardware ID matches the device ID of
        // my USB device.
        // ListBox1->Items->Add((char*)PropertyValueBuffer);
        DeviceIDFromRegistry = StrPas((char *)PropertyValueBuffer);
        // free(PropertyValueBuffer); //No longer need the PropertyValueBuffer,free
        // the memory to
        // prevent potential memory leaks
        delete PropertyValueBuffer; // No longer need the PropertyValueBuffer,free
        // the memory to
        // prevent potential memory leaks
        // Convert both strings to lower case.  This makes the code more
        // robust/portable accross OS
        // Versions
        ...
        if (MatchFound == true)
        {
            // Device must have been found.  Open read and write handles.  In order to
            // do this,we
            // will need the actual device path first.
            // We can get the path by calling
            // SetupDiGetDeviceInterfaceDetail(),however,we have to
            // call this function twice:  The first
            // time to get the size of the required structure/buffer to hold the
            // detailed interface
            // data,then a second time to actually
            // get the structure (after we have allocated enough memory for the
            // structure.)
            ...
            DetailedInterfaceDataStructure->cbSize = sizeof(SP_DEVICE_INTERFACE_DETAIL_DATA);
            // Now call SetupDiGetDeviceInterfaceDetail() a second time to receive the
            // goods.
            SetupDiGetDeviceInterfaceDetail(DeviceInfoTable, InterfaceDataStructure,
                                            DetailedInterfaceDataStructure, StructureSize, NULL,
                                            NULL);
            // We now have the proper device path,and we can finally open read and
            // write handles to
            // the device.
        ...
        InterfaceIndex++;
        // Keep looping until we either find a device with matching VID and PID,or
        // until we run out
        // of items.
    } // end of while(true)
    // ShowMessage("Sortie");
...
bool TPoKeys55::Write(unsigned char c, unsigned char b3, unsigned char b4, unsigned char b5)
{
    ...
    OutputBuffer[0] = 0; // The first byte is the "Report ID" and does not get
    ...
    // The basic Windows I/O functions WriteFile() and ReadFile() can be used to
    // read and write to
    // HID class USB devices
    //(once we have the read and write handles to the device, which are obtained
    // with CreateFile()).
    ...
    // Read(); //odczyt trzeba zrobiæ inaczej - w tym miejscu bêdzie za szybko i
    // nic siê nie odczyta
}
//---------------------------------------------------------------------------
...
{
    DWORD BytesRead = 0;
    InputBuffer[0] = 0; // The first byte is the "Report ID" and does not get
    // transmitted over the
    // USB bus.  Always set=0.
    // Now get the response packet from the firmware.
    // The following call to ReadFIle() retrieves 64 bytes of data from the USB
    // device.
...
{ // zwraca numer wersji, funkcja nieoptymalna
    // czasowo (czeka na odpowiedŸ)
    if (!WriteHandle)
        return "";
    Write(0x00, 0); // 0x00 - Read serial number, version
    if (ReadLoop(10))
    { // 3: serial MSB; 4: serial LSB; 5: software version
        // (v(1+[4-7]).([0-3])); 6: revision number
        ...
        /* //Ra: pozyskiwanie daty mo¿na sobie darowaæ, jest poniek¹d bez sensu
          Write(0x04,0); //0x04 - Read build date: drugi argument zmieniaæ od 0 do
          2, uzyskuj¹c
          kolejno po 4 znaki
          if (ReadLoop(5))
          {//2: 0x04; 3-6: char 1-4, 5-8, 9-11;
          (83-65-112-32-32-49-32-50-48-49-49-0=="Sep  1 2011")
           s+=" ("+AnsiString((char*)InputBuffer+3,4);
           Write(0x04,1); //0x04 - Read build date: drugi argument zmieniaæ od 0 do
          2, uzyskuj¹c
          kolejno po 4 znaki
           if (ReadLoop(5))
           {s+=AnsiString((char*)InputBuffer+3,4);
            Write(0x04,2); //0x04 - Read build date: drugi argument zmieniaæ od 0 do
          2, uzyskuj¹c
          kolejno po 4 znaki
            if (ReadLoop(5))
...
{ // funkcja powinna byæ wywo³ywana
    // regularnie, np. raz w ka¿dej ramce
    // ekranowej
    if (pause)
    { // specjalna procedura, jeœli utracone po³¹czenie spowodowa³o
        // pauzê
    ...
    else if (iRepeats >= 10) // youBy 2014-07: przy 5 powtórzeniach sieje mi pauz¹
    // po 2 razy na
