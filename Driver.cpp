...
// 2. hamowanie na zadanym odcinku do zadanej predkosci (ze stabilizacja
// przyspieszenia)
...
// 7. przejscie na zestaw spokojny jesli wystepuje duzo poslizgow lub wybic
// nadmiarowego.
...
AnsiString StopReasonTable[] = {
    ...
    "A command", // stopComm, //otrzymano tak¹ komendê (niewiadomego
    // pochodzenia)
    "Out of station", // stopOut, //komenda wyjazdu poza stacjê (raczej nie
    // powinna zatrzymywaæ!)
...
{ // sprawdzenie typu komendy w evencie i
    // okreœlenie prêdkoœci
    ...
    { // prêdkoœæ manewrow¹ zapisaæ, najwy¿ej AI
        // zignoruje przy analizie tabelki
        fVelNext = value1; // powinno byæ value2, bo druga okreœla "za"?
        iFlags |= 0x200;
    }
    else if (command == cm_SetVelocity)
    { // w semaforze typu "m" jest
        // ShuntVelocity dla Ms2 i SetVelocity
        // dla S1
        ...
        // SetVelocity 40 -1  -> PutValues: jechaæ 40 a¿ do miniêcia (koniec
        // ograniczenia(
        fVelNext = value1;
        ...
        if (value1 == 0.0) // jeœli pierwsza zerowa
            ...
            { // S1 na SBL, mo¿na przejechaæ po zatrzymaniu (tu nie mamy prêdkoœci ani
                // odleg³oœci)
                fVelNext = value2; // normalnie bêdzie zezwolenie na jazdê, aby siê
                // usun¹³ z tabelki
    ...
    { // w trybie manewrowym: skanowaæ od
        // niej wstecz i stan¹æ po
        // wyjechaniu za sygnalizator i
        // zmieniæ kierunek
    ...
    { // inna komenda w evencie skanowanym powoduje zatrzymanie i wys³anie
        // tej komendy
        iFlags &= ~0xE00; // nie manewrowa, nie przystanek, nie zatrzymaæ na SBL
...
{ // przeliczenie odleg³oœci od punktu (*p),
    // w kierunku (*dir), zaczynaj¹c od
    // pojazdu
    ...
    fDist = v.Length(); // d³ugoœæ wektora to odleg³oœæ pomiêdzy czo³em a sygna³em
    // albo pocz¹tkiem toru
    ...
    { // je¿eli liczymy wzglêdem pojazdu
        ...
        { // jeœli coœ jest z ty³u, to dok³adna odleg³oœæ nie ma ju¿ wiêkszego
            // znaczenia
            ...
            {
                iFlags ^= 32; // 32-miniêty - bêdziemy liczyæ odleg³oœæ wzglêdem
                // przeciwnego koñca
        ...
        else if (fDist < 50.0) // przy du¿ym k¹cie ³uku iloczyn skalarny bardziej
            // zani¿y odleg³oœæ
            // ni¿ ciêciwa
            fDist = iska; // ale przy ma³ych odleg³oœciach rzut na chwilow¹ prost¹
        // ruchu da
    ...
    if (fDist > 0.0) // nie mo¿e byæ 0.0, a przypadkiem mog³o by siê trafiæ i by³o by Ÿle
        ...
        { // jeœli obiekt nie zosta³ miniêty, mo¿na od niego zliczaæ narastaj¹co
            // (inaczej mo¿e byæ
        ...
        if (trTrack) // mo¿e byæ NULL, jeœli koniec toru (???)
        {
            ...
            { // jeœli skrzy¿owanie, ograniczyæ prêdkoœæ przy skrêcaniu
                ...
                if ((iFlags & 32) == 0) // jeœli nie wjecha³
                    if (trTrack->iNumDynamics > 0) // a skrzy¿owanie zawiera pojazd
                        fVelNext = 0.0; // to zabroniæ wjazdu (chyba ¿e ten z przodu te¿
                // jedzie prosto)
            }
            if (iFlags & 8) // jeœli odcinek zmienny
            {
                ...
                { // Ra: zak³adam, ¿e s¹ tylko 2 mo¿liwe stany
                    ...
                    // problem jest chyba, jeœli zwrotnica siê prze³o¿y zaraz po zjechaniu
                    // z niej
    ...
    { // odczyt komórki pamiêci najlepiej by by³o zrobiæ jako notyfikacjê, czyli
        // zmiana komórki
...
{ // zapamiêtanie zdarzenia
    ...
    evEvent = e;
    vPos = e->PositionGet(); // wspó³rzêdne eventu albo komórki pamiêci (zrzutowaæ
    // na tor?)
    CommandCheck(); // sprawdzenie typu komendy w evencie i okreœlenie prêdkoœci
...
{ // sprawdzanie eventów na podanym torze
    // do podstawowego skanowania
    TEvent *e = (fDirection > 0) ? Track->evEvent2 : Track->evEvent1;
    if (!e)
        return NULL;
    if (e->bEnabled)
        return NULL;
    // jednak wszystkie W4 do tabelki, bo jej czyszczenie na przystanku wprowadza
    // zamieszanie
    return e;
}
...
{ // zwiêkszenie u¿ytej tabelki o jeden rekord
    ...
    // TODO: jeœli ostatnia pozycja zajêta, ustawiaæ dodatkowe flagi - teraz jest
    // to w
    // TableTraceRoute()
    // TODO: przyda³o by siê te¿ posortowaæ tabelkê wg odleg³oœci (ale nie w tym
    // miejscu)
    return true; // false gdy siê na³o¿y
};

bool TController::TableNotFound(TEvent *e)
{ // sprawdzenie, czy nie zosta³ ju¿
    // dodany do tabelki (np. podwójne
    // W4 robi problemy)
...
{ // skanowanie trajektorii na odleg³oœæ
    // (fDistance) od (pVehicle) w kierunku przodu
    // sk³adu i
    ...
    { // jeœli zmiana kierunku, zaczynamy od
        // toru ze wskazanym pojazdem
        ...
        iTableDirection = iDirection; // ustalenie w jakim kierunku jest wype³niana
        // tabelka wzglêdem pojazdu
        iFirst = iLast = 0;
        tLast = NULL; //¿aden nie sprawdzony
    }
    else
    { // kontynuacja skanowania od ostatnio sprawdzonego toru (w ostatniej
        // pozycji zawsze jest tor)
        ...
        { // jeœli zape³ni³a siê tabelka
            ...
            return; // znaleziono semafor lub tor z prêdkoœci¹ zero i nie ma co dalej
        // sprawdzaæ
        ...
        fTrackLength =
            sSpeedTable[iLast].iFlags & 0x60 ? 0.0 : pTrack->Length(); // nie doliczaæ d³ugoœci gdy:
        // 32-miniêty pocz¹tek,
        // 64-jazda do koñca toru
    }
    if (fCurrentDistance < fDistance)
    { // jeœli w ogóle jest po co analizowaæ
        --iLast; // jak coœ siê znajdzie, zostanie wpisane w tê pozycjê, któr¹
        // w³aœnie odczytano
        while (fCurrentDistance < fDistance)
        {
            ...
            { // jeœli tor nie by³ jeszcze sprawdzany
                ...
                { // trzeba sprawdziæ tabelkê, bo dodawanie drugi raz tego samego
                    // przystanku nie
                    // jest korzystne
                ...
                } // event dodajemy najpierw, ¿eby móc sprawdziæ, czy tor zosta³ dodany
                // po
                ...
                { // odcinek dodajemy do tabelki, gdy jest istotny dla ruchu
                    if (TableAddNew())
                    { // teraz dodatkowo zapamiêtanie wybranego
                        // segmentu dla skrzy¿owania
                        ...
                        { // dopiero tutaj jest ustalany kierunek segmentu na skrzy¿owaniu
                            ...
                            sSpeedTable[iLast].iFlags &=
                                ~4; // usuniêcie flagi kierunku, bo mo¿e byæ b³êdna
                            if (sSpeedTable[iLast].iFlags < 0)
                                sSpeedTable[iLast].iFlags |= 4; // ustawienie flagi kierunku na
                            // podstawie wybranego segmentu
                            if (int(fLastDir) * sSpeedTable[iLast].iFlags < 0)
                                fLastDir = -fLastDir;
                            if (AIControllFlag) // dla AI na razie losujemy kierunek na
                                // kolejnym
                                // skrzy¿owaniu
                ...
                { // albo dla liczenia odleg³oœci przy pomocy ciêciw - te usuwaæ po
                    // przejechaniu
                    if (TableAddNew())
            ...
            fCurrentDistance += fTrackLength; // doliczenie kolejnego odcinka do
            // przeskanowanej d³ugoœci
            ...
            /*
               ...
               {//jeœli szukanie od Point1 w kierunku Point2
                ...
                if (pTrack) //jeœli dalej brakuje toru, to zostajemy na tym samym, z
               t¹ sam¹
               orientacj¹
                 if (tLast->iNextDirection)
                  fLastDir=-fLastDir; //mo¿na by zamiêtaæ i zmieniæ tylko jeœli jest
               pTrack
               }
            ...
            */
            ...
            { // jeœli kolejny istnieje
                if (tLast)
                    ...
                    { // jeœli kolejny ma wiêksz¹
                        // prêdkoœæ ni¿ poprzedni, to
                        // zapamiêtaæ poprzedni
                        // (do czasu wyjechania)
                ...
                { // jest ryzyko nieznalezienia ograniczenia - ograniczyæ prêdkoœæ do
                    // pozwalaj¹cej
            ...
            { // definitywny koniec skanowania, chyba ¿e dalej puszczamy
                // samochód po gruncie...
                if (TableAddNew()) // kolejny, bo siê cofnêliœmy o 1
                    sSpeedTable[iLast].Set(
                        tLast, fCurrentDistance,
                        fLastDir < 0 ? 0x45 : 0x41); // zapisanie ostatniego sprawdzonego toru
                return; // to ostatnia pozycja, bo NULL nic nie da, a mo¿e siê podpi¹æ
                // obrotnica,
...
{ // przeliczenie odleg³oœci w
    // tabelce, ewentualnie
    // doskanowanie (bez analizy
    // prêdkoœci itp.)
    if (iTableDirection != iDirection)
    ...
    { // trzeba sprawdziæ, czy coœ siê zmieni³o
        ...
        { // aktualizacja rekordów z wyj¹tkiem ostatniego
            if (sSpeedTable[i].iFlags & 1) // jeœli pozycja istotna
            {
                ...
                if (sSpeedTable[i].iFlags & 2) // jeœli odcinek
                {
                    ...
                    { // degradacja pozycji
                        sSpeedTable[i].iFlags &= ~1; // nie liczy siê
                             ...
                             0x20) // jest z ty³u (najechany) i nie jest zwrotnic¹ ani
                        // skrzy¿owaniem
                ...
                else if (sSpeedTable[i].iFlags & 0x100) // jeœli event
                {
                    if (sSpeedTable[i].fDist < (sSpeedTable[i].evEvent->Type == tp_PutValues ?
                        ...
                        { // poci¹g staje zawsze, a samochód tylko
                            // jeœli nie przejedzie ca³¹
                ...
                // if (sSpeedTable[i].fDist<-20.0*fLength) //jeœli to coœ jest 20 razy
                // dalej ni¿
                // d³ugoœæ sk³adu
            ...
            { // pozbycie siê pocz¹tkowej pozycji
                if ((sSpeedTable[i].iFlags & 1) ==
                    ...
                    iFirst = (iFirst + 1) % iSpeedTableSize; // kolejne sprawdzanie bêdzie
                // ju¿ od nastêpnej pozycji
            }
        }
...
{ // ustalenie parametrów, zwraca typ
    // komendy, jeœli sygna³ podaje
    // prêdkoœæ do jazdy
    ...
    { // sprawdzenie rekordów od (iFirst)
        // do (iLast), o ile s¹ istotne
        ...
        { // o ile dana pozycja tabelki jest istotna
            ...
            { // jeœli przystanek, trzeba obs³u¿yæ wg rozk³adu
                ...
                else if (iDrivigFlags & moveStopPoint) // jeœli pomijanie W4, to nie
                // sprawdza czasu odjazdu
                { // tylko gdy nazwa zatrzymania siê zgadza
                    ...
                    { // jeœli nie ma tu postoju
                        ...
                        if (sSpeedTable[i].fDist < 200.0) // przy 160km/h jedzie 44m/s, to
                        // da dok³adnoœæ rzêdu 5 sekund
                        { // zaliczamy posterunek w pewnej odleg³oœci przed (choæ W4 nie
// zas³ania
// ju¿ semafora)
#if LOGSTOPS
                    ...
                    { // zatrzymanie na W4
                        ...
                        // ((iDrivigFlags&moveStopCloser)?sSpeedTable[i].fDist<=fMaxProximityDist*(AIControllFlag?1.0:10.0):true)
                                     ...
                                     true)
                        // Ra 2F1I: odleg³oœæ plus d³ugoœæ poci¹gu musi byæ mniejsza od
                        // d³ugoœci
                        // peronu, chyba ¿e poci¹g jest d³u¿szy, to wtedy minimalna
                        // jeœli d³ugoœæ peronu ((sSpeedTable[i].evEvent->ValueGet(2)) nie
                        // podana,
                        ...
                        { // jeœli siê zatrzyma³ przy W4, albo sta³ w momencie zobaczenia W4
                            ...
                            { // drzwi otwieraæ jednorazowo
                                ...
                                { // otwieranie drzwi w sk³adach wagonowych - docelowo
                                    // wysy³aæ
                                    ...
                                    while (p)
                                    { // otwieranie drzwi w pojazdach - flaga zezwolenia
                                        // by³a by
                                        // lepsza
                                ...
                                if (fStopTime > -5) // na koñcu rozk³adu siê ustawia 60s i tu by
                                    // by³o skrócenie
                            ...
                            { // jeœli s¹ dalsze stacje,
                                // czekamy do godziny odjazdu
                                if (TrainParams->IsTimeToGo(GlobalTime->hh, GlobalTime->mm))
                                { // z dalsz¹ akcj¹
                                    // czekamy do godziny
                                    // odjazdu
                                    // if (TrainParams->CheckTrainLatency()<0.0) //jak siê ma
                                    // odjazd
                                    // do czasu odjazdu?
                                    ...
                                    TrainParams->StationIndexInc(); // przejœcie do nastêpnej
                                    asNextStop = TrainParams->NextStop(); // pobranie kolejnego
// miejsca zatrzymania
// TableClear(); //aby od nowa sprawdzi³o W4 z inn¹ nazw¹ ju¿ - to nie jest
// dobry pomys³
#if LOGSTOPS
                            ...
                            { // jeœli dojechaliœmy do koñca rozk³adu
                                ...
                                iDrivigFlags &=
                                    ~(moveStopCloser | moveStopPoint); // ma nie podje¿d¿aæ pod
                                // W4 i ma je pomijaæ
                                sSpeedTable[i].iFlags =
                                ...
                                JumpToNextOrder(); // wykonanie kolejnego rozkazu
                                // (Change_direction
                                // albo Shunt)
            ...
            if (sSpeedTable[i].iFlags &
                8) // zwrotnice s¹ usuwane z tabelki dopiero po zjechaniu z nich
                iDrivigFlags |= moveSwitchFound; // rozjazd z przodu/pod ogranicza np.
            // sens skanowania wstecz
            ...
            { // je¿eli event, mo¿e byæ potrzeba wys³ania komendy, aby ruszy³
                ...
                { // jeœli W5, to reakcja zale¿na od trybu jazdy
                    ...
                    { // w trybie poci¹gowym: mo¿na
                        // przyspieszyæ do wskazanej
                        // prêdkoœci (po
                        // zjechaniu z rozjazdów)
                        v = -1.0; // ignorowaæ?
                        if (sSpeedTable[i].fDist < 0.0) // jeœli wskaŸnik zosta³ miniêty
                        {
                            VelSignal = v; //!!! ustawienie, gdy przejechany jest lepsze ni¿ wcale,
                    ...
                    { // w trybie manewrowym: skanowaæ od niego wstecz, stan¹æ po
                        // wyjechaniu za
                        // sygnalizator i zmieniæ kierunek
                        v = 0.0; // zmiana kierunku mo¿e byæ podanym sygna³em, ale wypada³o
                        // by
                ...
                { // jeœli S1 na SBL
                    if (mvOccupied->Vel < 2.0) // stan¹æ nie musi, ale zwolniæ przynajmniej
                        if (sSpeedTable[i].fDist < fMaxProximityDist) // jest w maksymalnym zasiêgu
                        {
                            ...
                            // zbyt blisko
                            // usun¹æ flagê po podjechaniu blisko semafora zezwalaj¹cego na
                            // jazdê
                            // ostro¿nie interpretowaæ sygna³y - semafor mo¿e zezwalaæ na
                            // jazdê
                            // poci¹gu z przodu!
                        }
                ...
                if ((mvOccupied->CategoryFlag & 1) ?
                    ...
                    v = 0.0; // to mo¿e byæ podany dla tamtego: jechaæ tak, jakby tam stop
                // by³
                else
                { // zawalidrogi nie ma (albo pojazd jest samochodem), sprawdziæ
                    // sygna³
                    if (sSpeedTable[i].iFlags & 0x200) // jeœli Tm - w zasadzie to sprawdziæ
                    // komendê!
                    { // jeœli podana prêdkoœæ manewrowa
                        if ((OrderCurrentGet() & Obey_train) ? v == 0.0 : false)
                        { // jeœli tryb poci¹gowy a tarcze ma ShuntVelocity
                            // 0 0
                        ...
                        else if (go <= cm_Ready) // jeœli jeszcze nie ma komendy
                            if (v != 0.0) // komenda jest tylko gdy ma jechaæ, bo stoi na podstawie
                            // tabelki
                            { // jeœli nie by³o komendy wczeœniej - pierwsza siê liczy -
                                // ustawianie
                                // VelSignal
                                go = cm_ShuntVelocity; // w trybie poci¹gowym tylko jeœli w³¹cza
                                // tryb manewrowy (v!=0.0)
                                // Ra 2014-06: (VelSignal) nie mo¿e byæ tu ustawiane, bo Tm mo¿e
                                // byæ
                                // daleko
                    ...
                    else // if (sSpeedTable[i].iFlags&0x100) //jeœli semafor !!! Komendê
                        // trzeba
                        // sprawdziæ !!!!
                        if (go <= cm_Ready) // jeœli nie by³o komendy wczeœniej - pierwsza
                        // siê liczy
                        // - ustawianie VelSignal
                        if (v < 0.0 ? true : v >= 1.0) // bo wartoœæ 0.1 s³u¿y do hamowania tylko
                        {
                            go = cm_SetVelocity; // mo¿e odjechaæ
                            // Ra 2014-06: (VelSignal) nie mo¿e byæ tu ustawiane, bo semafor
                            // mo¿e
                            // byæ daleko
                        ...
                        { // jeœli
                            ...
                            if (iDrivigFlags &
                                ...
                                go = cm_Command; // komenda z komórki, do wykonania po
                            // zatrzymaniu
            ...
            { // pozycje z prêdkoœci¹ -1 mo¿na spokojnie pomijaæ
                ...
                else if (sSpeedTable[i].iFlags & 2) // jeœli tor
                { // tor ogranicza prêdkoœæ, dopóki ca³y sk³ad nie przejedzie,
                    // d=fLength+d; //zamiana na d³ugoœæ liczon¹ do przodu
                    if (v >= 1.0) // EU06 siê zawiesza³o po dojechaniu na koniec toru
                        // postojowego
                        if (d < -fLength)
                            continue; // zapêtlenie, jeœli ju¿ wyjecha³ za ten odcinek
                    if (v < fVelDes)
                        fVelDes = v; // ograniczenie aktualnej prêdkoœci a¿ do wyjechania za
                    // ograniczenie
                    // if (v==0.0) fAcc=-0.9; //hamowanie jeœli stop
                    continue; // i tyle wystarczy
                }
                else // event trzyma tylko jeœli VelNext=0, nawet po przejechaniu (nie
                    // powinno
                    // dotyczyæ samochodów?)
                    a = (v == 0.0 ? -1.0 : fAcc); // ruszanie albo hamowanie
                if (a < fAcc)
                { // mniejsze przyspieszenie to mniejsza mo¿liwoœæ
                    // rozpêdzenia siê albo koniecznoœæ
                    // hamowania
                ...
                { // jeœli nie ma wskazañ do hamowania, mo¿na
                    // podaæ drogê i prêdkoœæ na jej koñcu
                    fNext = v; // istotna jest prêdkoœæ na koñcu tego odcinka
...
{ // odtykacz: usuwa mniej istotne pozycje ze
    // œrodka tabelki, aby unikn¹æ zatkania
    //(np. brak ograniczenia pomiêdzy zwrotnicami, usuniête sygna³y, miniête
    // odcinki ³uku)
    ...
    { // sprawdzenie rekordów od (iFirst)
        ...
        if ((sSpeedTable[i].iFlags & 1) ?
                ...
                true)
        { // jeœli jest to miniêty (0x20) tor (0x03) do liczenia
            // ciêciw (0x80), a nie zwrotnica
            // (0x08)
    ...
    // jeœli powy¿sze odtykane nie pomo¿e, mo¿na usun¹æ coœ wiêcej, albo
    // powiêkszyæ tabelkê
                         ...
                         )
{
...
#if LOGPRESS == 0
        LogFile << AnsiString(" Time [s]   Velocity [m/s]  Acceleration [m/ss]   "
                              "Coupler.Dist[m]  "
                              "Coupler.Force[N]  TractionForce [kN]  FrictionForce [kN]   "
                              "BrakeForce [kN]    BrakePress [MPa]   PipePress [MPa]   "
                              "MotorCurrent [A]    MCP SCP BCP LBP DmgFlag Command CVal1 "
                              "CVal2")
                       .c_str() << "\r\n";
#endif
#if LOGPRESS == 1
    ...
    */
    ...
    { // samochody: na podst.
        ...
        // fDriverBraking=0.0065; //mno¿one przez (v^2+40*v) [km/h] daje prawie
        // drogê hamowania [m]
        fDriverBraking = 0.03; // coœ nie hamuj¹ te samochody zbyt dobrze
        fDriverDist = 5.0; // 5m - zachowywany odstêp przed kolizj¹
        fVelPlus = 10.0; // dopuszczalne przekroczenie prêdkoœci na ograniczeniu bez
        // hamowania
    ...
    { // poci¹gi i statki
        ...
        fVelPlus = 5.0; // dopuszczalne przekroczenie prêdkoœci na ograniczeniu bez
        // hamowania
    ...
    iStationStart = 0; // nic?
    // fAccThreshold mo¿e podlegaæ uczeniu siê - hamowanie powinno byæ
    // rejestrowane, a potem
    // analizowane
    fAccThreshold =
    ...
    iCoupler = 0; // sprzêg; niezerowy gdy ma byæ pod³¹czanie; samo pod³¹czanie w
    // trybie Connect
    // (wczeœniej mo¿e byæ np. Prepare_engine)
    fOverhead1 = 3000.0; // informacja o napiêciu w sieci trakcyjnej (0=brak
    // drutu, zatrzymaj!)
    fOverhead2 = -1.0; // informacja o sposobie jazdy (-1=normalnie, 0=bez pr¹du,
    // >0=z opuszczonym i
    ...
    // podniesionymi pantografami
    fAccDesiredAv = 0.0; // uœrednione przyspieszenie z kolejnych przeb³ysków
    // œwiadomoœci, ¿eby
    // ograniczyæ migotanie
    fVoltage = 0.0; // uœrednione napiêcie sieci: przy spadku poni¿ej wartoœci
    // minimalnej opóŸniæ
...
{ // czyszczenie tabeli rozkazów na starcie albo
    // po dojœciu do koñca
    OrderPos = 0;
...
{ // umieszczenie obsady w odpowiednim cz³onie,
    // wykonywane wy³¹cznie gdy steruje AI
    iDirection = iDirectionOrder; // kierunek (wzglêdem sprzêgów pojazdu z AI)
    // w³aœnie zosta³
    // ustalony (zmieniony)
    if (iDirection)
    { // jeœli jest ustalony kierunek
        TDynamicObject *old = pVehicle, *d = pVehicle; // w tym siedzi AI
        TController *drugi; // jakby by³y dwa, to zamieniæ miejscami, a nie robiæ
        // wycieku pamiêci
        // poprzez nadpisanie
                     ...
                     ctrain_controll))
        {
            ...
            //   mvOccupied->BrakeLevelSet((mvOccupied->BrakeHandle==FVel6)?4:-2);
            //   //odciêcie na
        ...
        { // jeœli zmieniony zosta³ pojazd prowadzony
            ...
            ControllingSet(); // utworzenie po³¹czenia do sterowanego pojazdu (mo¿e
            // siê zmieniæ) -
            // silnikowy dla EZT
        }
        if (mvControlling->EngineType == DieselEngine) // dla 2Ls150 - przed
            // ustawieniem kierunku -
            // mo¿na zmieniæ tryb pracy
        ...
        mvOccupied->ActiveCab = iDirection; // aktywacja kabiny w prowadzonym
        // poje¿dzie (silnikowy mo¿e byæ
        // odwrotnie?)
        ...
        if (brake) // hamowanie tylko jeœli by³ wczeœniej zahamowany (bo mo¿liwe, ¿e
            // jedzie!)
...
{ // autorewident: nastawianie hamulców w
    ...
    { // jeœli s¹ wagony
        ...
        // zasadniczo na sieci PKP kilka lat temu na P/GP jeŸdzi³y tylko
        // kontenerowce o
    ...
    { // 3. Nastawianie
        ...
        case bdelay_P: // towarowy P - lokomotywa na G, reszta na P.
            d->MoverParameters->BrakeDelaySwitch(d->MoverParameters->Power > 1 ? bdelay_G :
                                                                                 bdelay_P);
            break;
        case bdelay_G: // towarowy G - wszystko na G, jeœli nie ma to P (powinno siê
            // wy³¹czyæ
            // hamulec)
            d->MoverParameters->BrakeDelaySwitch(
                TestFlag(d->MoverParameters->BrakeDelays, bdelay_G) ? bdelay_G : bdelay_P);
            break;
        case bdelay_R: // towarowy GP - lokomotywa oraz 5 pierwszych pojazdów przy
            // niej na G, reszta
            // na P
...
{ // sprawdzenie stanu posiadanych
    // pojazdów w sk³adzie i
    // zapalenie œwiate³
    ...
    // Ra 2014-09: ustawiæ moveMultiControl, jeœli wszystkie s¹ w ukrotnieniu (i
    // skrajne maj¹
    // kabinê?)
    while (p)
    { // sprawdzanie, czy jest g³ównym steruj¹cym, ¿eby nie by³o konfliktu
        ...
        /* //youBy: bez przesady, to jest proteza, napelniac mozna, a nawet trzeba,
          ale z umiarem!
    ...
    */
    ControllingSet(); // ustalenie cz³onu do sterowania (mo¿e byæ inny ni¿
    // zasiedziany)
    int pantmask = 1;
    if (iDrivigFlags & movePrimary)
    { // jeœli jest aktywnie prowadz¹cym pojazd,
        // mo¿e zrobiæ w³asny porz¹dek
        p = pVehicles[0];
        while (p)
        {
            if (TrainParams)
            ...
            { // jeœli pojazd posiada pantograf, to przydzielamy
                // mu maskê, któr¹ bêdzie informowa³ o
                // jeŸdzie bezpr¹dowej
                p->iOverheadMask = pantmask;
                pantmask << 1; // przesuniêcie bitów, max. 32 pojazdy z pantografami w
                // sk³adzie
            }
        ...
        { // jeœli prowadzi komputer
        ...
        else // Ra 2014-02: lepiej tu ni¿ w pêtli obs³uguj¹cej komendy, bo tam siê
            // zmieni informacja
            // o sk³adzie
            ...
            case Change_direction:
                while (OrderCurrentGet() & (Change_direction))
                    JumpToNextOrder(); // zmianê kierunku te¿ mo¿na olaæ, ale zmieniæ
                // kierunek
                // skanowania!
                break;
            case Connect:
                ...
                { // jeœli mia³o byæ ³¹czenie,
                    // zak³adamy, ¿e jest dobrze
                    // (sprawdziæ?)
...
{ // ustawienie kierunku po wczytaniu
    // trainset (mo¿e jechaæ na wstecznym
...
{ // zmiana kierunku jazdy, niezale¿nie od kabiny
    int testd = newdir;
    if (Vehicle->Vel < 0.5)
    { // jeœli prawie stoi, mo¿na zmieniæ kierunek, musi
        // byæ wykonane dwukrotnie, bo za pierwszym
        // razem daje na zero
        switch (newdir * Vehicle->CabNo)
        { // DirectionBackward() i
        // DirectionForward() to zmiany wzglêdem
        // kabiny
    ...
    if (Vehicle->TrainType == dt_EZT)
        if (Vehicle->ActiveDir > 0)
            // if () //tylko jeœli jazda poci¹gowa (tego nie wiemy w momencie
            // odpalania silnika)
...
{ // ustawienie odczekania po
    // zatrzymaniu (ustawienie w
    // trakcie jazdy zatrzyma)
...
{ // ustawienie nowej prêdkoœci
    WaitingTime = -WaitingExpireTime; // przypisujemy -WaitingExpireTime, a potem
    // porównujemy z
    // zerem
    ...
    */
    ...
    else
    {
        ...
        if (OrderList[OrderPos] ?
            ...
            if ((mvOccupied->Vel < 1.0)) // jesli stoi (na razie, bo chyba powinien
                // te¿, gdy hamuje przed semaforem)
                    ...
                    if (!(iDrivigFlags & (moveStartHornDone | moveConnect))) // jeœli nie zatr¹bione
                        ...
                        if (mvOccupied->CategoryFlag & 1) // tylko poci¹gi tr¹bi¹ (unimogi tylko na
                            ...
                            { // fWarningDuration=0.3; //czas tr¹bienia
                                // if (AIControllFlag) //jak siedzi krasnoludek, to w³¹czy
                                // tr¹bienie
                                // mvOccupied->WarningSignal=pVehicle->iHornWarning; //wysokoœæ
                                // tonu
                                // (2=wysoki)
...
/* //funkcja do niczego nie potrzebna (ew. do przesuniêcia pojazdu o odleg³oœæ
NewDist)
...
{//informacja o prêdkoœci w pewnej odleg³oœci
 ...
 //if
((NewVelNext>=0)&&((VelNext>=0)&&(NewVelNext<VelNext))||(NewVelNext<VelSignal))||(VelNext<0))
 {MaxVelFlag=False; //Ra: to nie jest u¿ywane
  MinVelFlag=False; //Ra: to nie jest u¿ywane
  VelNext=NewVelNext;
  fProximityDist=NewDist; //dodatnie: przeliczyæ do punktu; ujemne: wzi¹æ
dos³ownie
  return true;
 }
 //else return false
    ...
    { // with Controlling do
        ...
        /*
          ...
          {//sprawdzenie jazdy na widocznoœæ
           TCoupling
          *c=pVehicles[0]->MoverParameters->Couplers+(pVehicles[0]->DirectionGet()>0?0:1);
          //sprzêg
          z przodu sk³adu
           if (c->Connected) //a mamy coœ z przodu
            if (c->CouplingFlag==0) //jeœli to coœ jest pod³¹czone sprzêgiem
          wirtualnym
            {//wyliczanie optymalnego przyspieszenia do jazdy na widocznoœæ (Ra: na
          pewno tutaj?)
             double k=c->Connected->Vel; //prêdkoœæ pojazdu z przodu (zak³adaj¹c, ¿e
          jedzie w tê
          sam¹ stronê!!!)
             if (k<=mvOccupied->Vel) //porównanie modu³ów prêdkoœci [km/h]
             {if (pVehicles[0]->fTrackBlock<fMaxProximityDist) //porównianie z
          minimaln¹ odleg³oœci¹
          kolizyjn¹
               k=-AccPreferred; //hamowanie maksymalne, bo jest za blisko
              else
              {//jeœli tamten jedzie szybciej, to nie potrzeba modyfikowaæ
          przyspieszenia
               double
          d=(pVehicles[0]->fTrackBlock-0.5*fabs(mvOccupied->V)-fMaxProximityDist);
          //bezpieczna odleg³oœæ za poprzednim
               ...
               //jeœli v2=50km/h,v1=60km/h,d=200m =>
          k=(192.9-277.8)/(25.92*(200-0.5*16.7)=-0.0171
          [m/s^2]
               //jeœli v2=50km/h,v1=60km/h,d=100m =>
          k=(192.9-277.8)/(25.92*(100-0.5*16.7)=-0.0357
          [m/s^2]
               //jeœli v2=50km/h,v1=60km/h,d=50m  => k=(192.9-277.8)/(25.92*(
          50-0.5*16.7)=-0.0786
          [m/s^2]
               //jeœli v2=50km/h,v1=60km/h,d=25m  => k=(192.9-277.8)/(25.92*(
          25-0.5*16.7)=-0.1967
          [m/s^2]
               if (d>0) //bo jak ujemne, to zacznie przyspieszaæ, aby siê zderzyæ
                k=(k*k-mvOccupied->Vel*mvOccupied->Vel)/(25.92*d); //energia
          kinetyczna dzielona
...
{ // odpalanie silnika
    ...
    {
        if (mvControlling->GetTrainsetVoltage()) // sprawdzanie, czy zasilanie jest
        // mo¿e w innym cz³onie
        {
            voltfront = true;
            voltrear = true;
        }
    ...
    //     begin
    //       if Couplers[0].Connected^.PantFrontVolt or
    //       Couplers[0].Connected^.PantRearVolt)
    //         voltfront:=true
    //       else
    //         voltfront:=false;
    //     end
    ...
    //     begin
    //      if Couplers[1].Connected^.PantFrontVolt or
    //      Couplers[1].Connected^.PantRearVolt)
    //        voltrear:=true
    //      else
    //        voltrear:=false;
    //     end
    ...
    { // czêœæ wykonawcza dla sterowania przez komputer
        ...
        { // jeœli silnikowy jest pantografuj¹cym
            ...
            { // za³¹czenie ma³ej sprê¿arki
                mvControlling->bPantKurek3 = false; // od³¹czenie zbiornika g³ównego, bo
                // z nim nie da rady napompowaæ
    ...
    { // najpierw ustalamy kierunek, jeœli nie zosta³ ustalony
        ...
        { // czêœæ wykonawcza dla sterowania przez komputer
            ...
            else if (!mvControlling->Mains)
            {
                ...
                { // Ra 2014-06: dla SN61 trzeba wrzuciæ pierwsz¹
                    // pozycjê - nie wiem, czy tutaj...
                    // kiedyœ dzia³a³o...
            ...
            { // Ra: iDirection okreœla, w któr¹ stronê jedzie sk³ad wzglêdem
                // sprzêgów pojazdu z AI
...
{ // wy³¹czanie silnika (test wy³¹czenia, a
    // czêœæ wykonawcza tylko jeœli steruje
    // komputer)
    bool OK = false;
    LastReactionTime = 0.0;
    ReactionTime = PrepareTime;
                    ...
                    if (AccDesired>-0.2) and ((Vel<20) or (Vel-VelNext<10)))
                            begin
                              if BrakeCtrlPos>0)
                               ...
                               OK:=IncLocalBrakeLevel(1);   //finezyjne hamowanie
                   lokalnym
                             end
                           else
                */
...
{ // zwiêkszenie prêdkoœci; zwraca false, jeœli
    // dalej siê nie da zwiêkszaæ
    ...
    if (iDrivigFlags & moveDoorOpened)
        Doors(false); // zamykanie drzwi - tutaj wykonuje tylko AI (zmienia
    // fActionTime)
    ...
    case ElectricSeriesMotor:
        ...
        if (!mvControlling->FuseFlag) //&&mvControlling->StLinFlag) //yBARC
            if ((mvControlling->MainCtrlPos == 0) ||
                ...
                // na pozycji 0 przejdzie, a na pozosta³ych bêdzie czekaæ, a¿ siê
                // za³¹cz¹ liniowe
                ...
                if (Ready || (iDrivigFlags & movePress))
                    ...
                    { // Ra: wywala³ nadmiarowy, bo Im
                        // mo¿e byæ ujemne; jak nie
                        // odhamowany, to nie
                        // przesadzaæ z pr¹dem
                        ...
                        { // bocznik na szeregowej przy ciezkich
                            // bruttach albo przy wysokim rozruchu
    ...
    case ElectricInductionMotor:
        if (!mvControlling->FuseFlag)
            if (Ready || (iDrivigFlags & movePress) ||
                (mvOccupied->ShuntMode)) //{(BrakePress<=0.01*MaxBrakePress)}
            {
                OK = mvControlling->IncMainCtrl(1);
            }
        break;
    ...
    case DieselEngine:
        if (mvControlling->ShuntModeAllow)
        { // dla 2Ls150 mo¿na zmieniæ tryb pracy,
            // jeœli jest w liniowym i nie daje
            // rady (wymaga
            // zerowania kierunku)
...
{ // zmniejszenie prêdkoœci (ale nie hamowanie)
    ...
    case None: // McZapkie-041003: wagon sterowniczy
        ...
        if (force) // przy aktywacji kabiny jest potrzeba natychmiastowego
            // wyzerowania
...
{ // Ra: regulacja prêdkoœci, wykonywana w ka¿dym
    // przeb³ysku œwiadomoœci AI
    // ma dokrêcaæ do bezoporowych i zdejmowaæ pozycje w przypadku przekroczenia
    // pr¹du
    switch (mvOccupied->EngineType)
    {
    case None: // McZapkie-041003: wagon sterowniczy
            ...
            { // jak ju¿ mo¿na coœ poruszaæ, przetok roz³¹czaæ od razu
                ...
                { // jak ma jechaæ
                    ...
                    { // jak jest odhamowany
                        ...
                        {
                            if (mvControlling->MainCtrlPos &&
                                !mvControlling->StLinFlag) // jak niby jedzie, ale ma
                                // roz³¹czone liniowe
    ...
    case ElectricSeriesMotor:
        if ((!mvControlling->StLinFlag) && (!mvControlling->DelayCtrlFlag) &&
            ...
            //    if (iDrivigFlags&moveIncSpeed) {} //jeœli czeka na za³¹czenie
            //    liniowych
            //    else
            while (DecSpeed())
                ; // zerowanie napêdu
            ...
            { // procedura wje¿d¿ania na ekstremalne wzniesienia
                if (fabs(mvControlling->Im) >
                    ...
                    // if
                    // (mvControlling->Imin*mvControlling->Voltage/(fMass*fAccGravity)<-2.8)
                    // //a
                    ...
                    { // w³¹czenie wysokiego rozruchu;
                        ...
                        { // jeœli jedzie na równoleg³ym, to zbijamy do szeregowego,
                            // aby w³¹czyæ
                            // wysoki rozruch
            ...
            { // sprawdzenie, czy rozruch wysoki jest potrzebny
                ...
                //   if (Im<1.3*Imin) //lekkie przekroczenie miimalnego pr¹du jest
                //   dopuszczalne
                //    IncMainCtrl(1); //zwieksz nastawnik skoro mo¿esz - tak aby siê
                //    ustawic na
                //    bezoporowej
            }
        break;
    ...
    case DieselEngine:
        ...
        if (!mvControlling->MotorParam[mvControlling->ScndCtrlPos].AutoSwitch) // gdy biegi rêczne
            ...
            // if (mvControlling->enrot>0.95*mvControlling->dizel_nMmax) //youBy:
            // jeœli obroty >
            // 0,95 nmax, wrzuæ wy¿szy bieg - Ra: to nie dzia³a
            { // jak prêdkoœæ wiêksza ni¿ 0.6 maksymalnej na danym biegu, wrzuciæ
                // wy¿szy
            ...
            { // jak prêdkoœæ mniejsza ni¿ minimalna na danym
                // biegu, wrzuciæ ni¿szy
                mvControlling->DecMainCtrl(2);
        ...
        { // jeœli drzwi sterowane z kabiny
            if (AIControllFlag)
                ...
                { // AI zamyka drzwi przed odjazdem
                    ...
                    mvOccupied->DoorRight(false);
                    // Ra: trzeba by ustawiæ jakiœ czas oczekiwania na zamkniêcie siê
                    // drzwi
...
{ // odczytuje i wykonuje komendê
    // przekazan¹ lokomotywie
...
{ // wys³anie komendy przez event PutValues, jak pojazd
    // ma obsadê, to wysy³a tutaj, a nie do pojazdu
    // bezpoœrednio
    vector3 sl;
...
{ // analiza komendy
    ...
    { // SHP wyzwalane jest przez cz³on z obsad¹,
        // ale obs³ugiwane przez silnikowy
        // nie jest to najlepiej zrobione, ale bez symulacji obwodów lepiej nie
        // bêdzie
        // Ra 2014-04: jednak przenios³em do rozrz¹dczego
        mvOccupied->PutCommand(NewCommand, NewValue1, NewValue2, mvOccupied->Loc);
        mvOccupied->RunInternalCommand(); // rozpoznaj komende bo lokomotywa jej nie
        // rozpoznaje
        return true; // za³atwione
    }
    if (NewCommand == "Overhead")
    { // informacja o stanie sieci trakcyjnej
        fOverhead1 = NewValue1; // informacja o napiêciu w sieci trakcyjnej (0=brak
        // drutu, zatrzymaj!)
        fOverhead2 = NewValue2; // informacja o sposobie jazdy (-1=normalnie, 0=bez
        // pr¹du, >0=z
        // opuszczonym i ograniczeniem prêdkoœci)
        return true; // za³atwione
    }
    else if (NewCommand == "Emergency_brake") // wymuszenie zatrzymania,
    // niezale¿nie kto prowadzi
    ...
    { // przypisanie nowego rozk³adu
        // jazdy, równie¿ prowadzonemu
        // przez u¿ytkownika
        ...
        if (NewCommand != "none")
        {
            ...
            { // inicjacja pierwszego przystanku i pobranie jego nazwy
                ...
                { // wczytanie dŸwiêku odjazdu podawanego
                    // bezpoœrenido
                ...
                else
                {
                    ...
                    if (FileExists(NewCommand))
                    { // wczytanie dŸwiêku odjazdu w wersji
                        // radiowej (s³ychaæ tylko w kabinie)
                        tsGuardSignal = new TTextSound();
        ...
        if (NewLocation) // jeœli podane wspó³rzêdne eventu/komórki ustawiaj¹cej
        // rozk³ad (trainset
        // nie podaje)
        {
            ...
            /*
              if (NewValue1!=0.0) //jeœli ma jechaæ
               if (iDirectionOrder==0) //a kierunek nie by³ okreœlony (normalnie
              okreœlany przez
              reardriver/headdriver)
                iDirectionOrder=NewValue1>0?1:-1; //ustalenie kierunku jazdy wzglêdem
              sprzêgów
               else
            ...
            */
            if (AIControllFlag) // jeœli prowadzi komputer
                Activation(); // umieszczenie obs³ugi we w³aœciwym cz³onie, ustawienie
            // nawrotnika w
            // przód
        }
        /*
        ...
        */
        ...
        // if (mvOccupied->CabNo*mvOccupied->V*NewValue1<0) //jeœli zadana prêdkoœæ
        // niezgodna z
        // aktualnym kierunkiem jazdy
        //  DirectionForward(false); //jedziemy do ty³u (nawrotnik do ty³u)
        // CheckVehicles(); //sprawdzenie sk³adu, AI zapali œwiat³a
        TableClear(); // wyczyszczenie tabelki prêdkoœci, bo na nowo trzeba okreœliæ
        // kierunek i
        // sprawdziæ przystanki
        OrdersInit(fabs(NewValue1)); // ustalenie tabelki komend wg rozk³adu oraz
        // prêdkoœci pocz¹tkowej
        // if (NewValue1!=0.0) if (!AIControllFlag) DirectionForward(NewValue1>0.0);
        // //ustawienie
    ...
    if (NewCommand == "SetVelocity")
    {
        ...
        { // o ile jazda
            ...
            OrderNext(Obey_train); // to uruchomiæ jazdê poci¹gow¹ (od razu albo po
            // odpaleniu
            // silnika
            OrderCheck(); // jeœli jazda poci¹gowa teraz, to wykonaæ niezbêdne
            // operacje
        }
    ...
    { // uruchomienie jazdy manewrowej
        // b¹dŸ zmiana prêdkoœci
        if (NewLocation)
            vCommandLocation = *NewLocation;
        // if (OrderList[OrderPos]=Obey_train) and (NewValue1<>0))
        if (!iEngineActive)
            OrderNext(Prepare_engine); // trzeba odpaliæ silnik najpierw
        OrderNext(Shunt); // zamieniamy w aktualnej pozycji, albo dodajey za
        // odpaleniem silnika
        if (NewValue1 != 0.0)
        {
            // if (iVehicleCount>=0) WriteLog("Skasowano ilosæ wagonów w
            // ShuntVelocity!");
            iVehicleCount = -2; // wartoœæ neutralna
            CheckVehicles(); // zabraæ to do OrderCheck()
        }
        // dla prêdkoœci ujemnej przestawiæ nawrotnik do ty³u? ale -1=brak
        // ograniczenia !!!!
        // if (iDrivigFlags&movePress) WriteLog("Skasowano docisk w
        // ShuntVelocity!");
    ...
    { // oczekiwanie; NewValue1 - czas
        // oczekiwania, -1 = na inn¹
        // komendê
    ...
    { // w³¹czenie albo wy³¹czenie
        // silnika (w szerokim sensie)
        ...
        // po za³¹czeniu przejdzie do kolejnej komendy, po wy³¹czeniu na
        // Wait_for_orders
    }
    else if (NewCommand == "Change_direction")
    {
        ...
        else if (NewLocation) // jeœli podane wspó³rzêdne eventu/komórki
        // ustawiaj¹cej rozk³ad
        // (trainset nie podaje)
        {
            vector3 v = *NewLocation - pVehicle->GetPosition(); // wektor do punktu steruj¹cego
            vector3 d = pVehicle->VectorFront(); // wektor wskazuj¹cy przód
        ...
        // Change_direction wykona siê samo i nastêpnie przejdzie do kolejnej
        // komendy
    }
    ...
    { // NewValue1 - iloœæ wagonów
        // (-1=wszystkie); NewValue2: 0=odczep,
        // 1..63=do³¹cz, -1=bez zmian
        //-3,-y - pod³¹czyæ do ca³ego stoj¹cego sk³adu (sprzêgiem y>=1), zmieniæ
        // kierunek i czekaæ w
        // trybie poci¹gowym
        //-2,-y - pod³¹czyæ do ca³ego stoj¹cego sk³adu (sprzêgiem y>=1), zmieniæ
        // kierunek i czekaæ
        //-2, y - pod³¹czyæ do ca³ego stoj¹cego sk³adu (sprzêgiem y>=1) i czekaæ
        //-1,-y - pod³¹czyæ do ca³ego stoj¹cego sk³adu (sprzêgiem y>=1) i jechaæ w
        // powrotn¹ stronê
        //-1, y - pod³¹czyæ do ca³ego stoj¹cego sk³adu (sprzêgiem y>=1) i jechaæ
        // dalej
        //-1, 0 - tryb manewrowy bez zmian (odczepianie z pozostawieniem wagonów nie
        // ma sensu)
        // 0, 0 - odczepienie lokomotywy
        // 1,-y - pod³¹czyæ siê do sk³adu (sprzêgiem y>=1), a nastêpnie odczepiæ i
        // zabraæ (x)
        // wagonów
        ...
        if (NewValue2 != 0) // jeœli podany jest sprzêg
        {
            ...
            else if (NewValue2 < 0.0) // jeœli wszystkie, a sprzêg ujemny, to tylko
            // zmiana kierunku
            // po podczepieniu
            { // np. Shunt -1 -3
            ...
            if (NewValue1 >= 0.0) // jeœli iloœæ wagonów inna ni¿ wszystkie
        { // bêdzie odczepianie, ale jeœli wagony s¹ z przodu, to trzeba najpierw
            // zmieniæ kierunek
        ...
        if (NewValue1 < -1.5) // jeœli -2/-3, czyli czekanie z ruszeniem na sygna³
            iDrivigFlags |= moveStopHere; // nie podje¿d¿aæ do semafora, jeœli droga
        // nie jest wolna
        // (nie dotyczy Connect)
        ...
        // if ((iVehicleCount>=0)&&(NewValue1<0)) WriteLog("Skasowano ilosæ wagonów
        // w Shunt!");
    ...
    else if (NewCommand == "Jump_to_order")
    {
        ...
        /*
          ...
          {
           append(AIlogFile);
           writeln(AILogFile,ElapsedTime:5:2," - new order: ",Order2Str(
          OrderList[OrderPos])," @
          ",OrderPos);
           close(AILogFile);
          }
    ...
    /* //ta komenda jest teraz skanowana, wiêc wysy³anie jej eventem nie ma sensu
     else if (NewCommand=="OutsideStation") //wskaznik W5
     {
      if (OrderList[OrderPos]==Obey_train)
       SetVelocity(NewValue1,NewValue2,stopOut); //koniec stacji - predkosc
     szlakowa
    ...
    */
    ...
    {
        if (AIControllFlag) // poni¿sza komenda nie jest wykonywana przez
            // u¿ytkownika
            if (NewValue1 > 0)
            {
    ...
    { // wybór kana³u radiowego (którego
        // powinien u¿ywaæ AI, rêczny
        // maszynista musi go ustawiæ sam)
        ...
        // NewValue2 mo¿e zawieraæ dodatkowo oczekiwany kod odpowiedzi, np. dla W29
        // "nawi¹zaæ
...
{ // zapis logu - na razie tylko wypisanie
    // parametrów
    if (LogFile.is_open())
    {
#if LOGPRESS == 0
...
{ // uruchamiaæ przynajmniej raz na sekundê
    ...
    double AbsAccS;
    // double VelReduced; //o ile km/h mo¿e przekroczyæ dozwolon¹ prêdkoœæ bez
    // hamowania
    bool UpdateOK = false;
    if (AIControllFlag)
    { // yb: zeby EP nie musial sie bawic z ciesnieniem w PG
        ...
        if (mvControlling->SlippingWheels)
        {
            ...
            // Controlling->SlippingWheels=false; //a to tu nie ma sensu, flaga
            // u¿ywana w dalszej
            // czêœci
        }
    }
    // ABu-160305 testowanie gotowoœci do jazdy
    // Ra: przeniesione z DynObj, sk³ad u¿ytkownika te¿ jest testowany, ¿eby mu
    // przekazaæ, ¿e ma
    // odhamowaæ
    ...
    { // sprawdzenie odhamowania wszystkich po³¹czonych pojazdów
        if (Ready) // bo jak coœ nie odhamowane, to dalej nie ma co sprawdzaæ
            // if
            // (p->MoverParameters->BrakePress>=0.03*p->MoverParameters->MaxBrakePress)
            if (p->MoverParameters->BrakePress >= 0.4) // wg UIC okreœlone sztywno na 0.04
            {
                Ready = false; // nie gotowy
                // Ra: odluŸnianie prze³adowanych lokomotyw, ci¹gniêtych na zimno -
                // prowizorka...
    ...
    if (iDirection)
        fAccGravity /= iDirection * fMass; // si³ê generuj¹ pojazdy na pochyleniu
    // ale dzia³a ona ca³oœæ sk³adu, wiêc
    // a=F/m
    if (!Ready) // v367: jeœli wg powy¿szych warunków sk³ad nie jest odhamowany
        if (fAccGravity < -0.05) // jeœli ma pod górê na tyle, by siê stoczyæ
            // if (mvOccupied->BrakePress<0.08) //to wystarczy, ¿e zadzia³aj¹ liniowe
            // (nie ma ich
            // jeszcze!!!)
    ...
    if (AIControllFlag)
    {
        if (mvControlling->EnginePowerSource.SourceType == CurrentCollector)
        {
            if (mvOccupied->ScndPipePress > 4.3) // gdy g³ówna sprê¿arka bezpiecznie nabije
                // ciœnienie
                mvControlling->bPantKurek3 = true; // to mo¿na przestawiæ kurek na
            // zasilanie pantografów z g³ównej
            // pneumatyki
            fVoltage =
                0.5 * (fVoltage +
            ...
            if (fVoltage < mvControlling->EnginePowerSource.CollectorParameters
                ...
                if (fActionTime >= 0) // jeœli czas oczekiwania nie zosta³ ustawiony
                    fActionTime = -2 - random(10); // losowy czas oczekiwania przed
            // ponownym za³¹czeniem jazdy
        }
        if (mvOccupied->Vel > 0.0)
        { // je¿eli jedzie
            if (iDrivigFlags & moveDoorOpened) // jeœli drzwi otwarte
                if (mvOccupied->Vel > 1.0) // nie zamykaæ drzwi przy drganiach, bo zatrzymanie na W4
                    ...
                    Doors(false);
            // przy prowadzeniu samochodu trzeba ka¿d¹ oœ odsuwaæ oddzielnie, inaczej
            // kicha wychodzi
            if (mvOccupied->CategoryFlag & 2) // jeœli samochód
                // if (fabs(mvOccupied->OffsetTrackH)<mvOccupied->Dim.W) //Ra: szerokoœæ
                // drogi tu
                // powinna byæ?
    ...
    ElapsedTime += dt;
    WaitingTime += dt;
    fBrakeTime -= dt; // wpisana wartoœæ jest zmniejszana do 0, gdy ujemna nale¿y
    // zmieniæ nastawê hamulca
    ...
    {
        // Ra: nie wiem czemu ReactionTime potrafi dostaæ 12 sekund, to jest
        // przegiêcie, bo prze¿yna
        // STÓJ
        // yB: otó¿ jest to jedna trzecia czasu nape³niania na towarowym; mo¿e siê
        // przydaæ przy
        // wdra¿aniu hamowania, ¿eby nie rusza³o kranem jak g³upie
        // Ra: ale nie mo¿e siê budziæ co pó³ minuty, bo prze¿yna semafory
        // Ra: trzeba by tak:
        // 1. Ustaliæ istotn¹ odleg³oœæ zainteresowania (np. 3×droga hamowania z
        // V.max).
        fBrakeDist = fDriverBraking * mvOccupied->Vel *
                     (40.0 + mvOccupied->Vel); // przybli¿ona droga hamowania
        // dla hamowania -0.2 [m/ss] droga wynosi 0.389*Vel*Vel [km/h], czyli 600m
        // dla 40km/h, 3.8km
        // dla 100km/h i 9.8km dla 160km/h
        // dla hamowania -0.4 [m/ss] droga wynosi 0.096*Vel*Vel [km/h], czyli 150m
        // dla 40km/h, 1.0km
        // dla 100km/h i 2.5km dla 160km/h
        // ogólnie droga hamowania przy sta³ym opóŸnieniu to Vel*Vel/(3.6*3.6*a) [m]
        // fBrakeDist powinno byæ wyznaczane dla danego sk³adu za pomoc¹ sieci
        // neuronowych, w
        // zale¿noœci od prêdkoœci i si³y (ciœnienia) hamowania
        // nastêpnie w drug¹ stronê, z drogi hamowania i chwilowej prêdkoœci powinno
        // byæ wyznaczane
        // zalecane ciœnienie
        if (fMass > 1000000.0)
        ...
        // koniecznie nale¿y
        // wyd³u¿yæ drogê na czas
        // reakcji
        // double
        // scanmax=(mvOccupied->Vel>0.0)?3*fDriverDist+fBrakeDist:10.0*fDriverDist;
        double scanmax = (mvOccupied->Vel > 5.0) ?
                             400 + fBrakeDist :
                             50.0 * fDriverDist; // 1000m dla stoj¹cych poci¹gów; Ra 2015-01: przy
        // d³u¿szej drodze skanowania AI jeŸdzi spokojniej
        // 2. Sprawdziæ, czy tabelka pokrywa za³o¿ony odcinek (nie musi, jeœli jest
        // STOP).
        // 3. Sprawdziæ, czy trajektoria ruchu przechodzi przez zwrotnice - jeœli
        // tak, to sprawdziæ,
        // czy stan siê nie zmieni³.
        // 4. Ewentualnie uzupe³niæ tabelkê informacjami o sygna³ach i
        // ograniczeniach, jeœli siê
        // "zu¿y³a".
        ...
        // 6. Z tabelki wyznaczyæ krytyczn¹ odleg³oœæ i prêdkoœæ (najmniejsze
        // przyspieszenie).
        // 7. Jeœli jest inny pojazd z przodu, ewentualnie skorygowaæ odleg³oœæ i
        // prêdkoœæ.
        // 8. Ustaliæ czêstotliwoœæ œwiadomoœci AI (zatrzymanie precyzyjne -
        // czêœciej, brak atrakcji
        // - rzadziej).
        if (AIControllFlag)
        { // tu bedzie logika sterowania
            if (mvOccupied->CommandIn.Command != "")
                if (!mvOccupied->RunInternalCommand()) // rozpoznaj komende bo
                    // lokomotywa jej nie
                    // rozpoznaje
        ...
        { // ustalenie prêdkoœci przy doczepianiu i
        // odczepianiu, dystansów w pozosta³ych
        // przypadkach
        case Connect: // pod³¹czanie do sk³adu
            if (iDrivigFlags & moveConnect)
            { // jeœli stan¹³ ju¿ blisko, unikaj¹c
                // zderzenia i mo¿na próbowaæ pod³¹czyæ
                ...
                if (AIControllFlag)
                { // to robi tylko AI, wersjê dla cz³owieka trzeba
                    // dopiero zrobiæ
                    // sprzêgi sprawdzamy w pierwszej kolejnoœci, bo jak po³¹czony, to
                    // koniec
                    bool ok; // true gdy siê pod³¹czy (uzyskany sprzêg bêdzie zgodny z
                    // ¿¹danym)
                    if (pVehicles[0]->DirectionGet() > 0) // jeœli sprzêg 0
                    { // sprzêg 0 - próba podczepienia
                        if (pVehicles[0]->MoverParameters->Couplers[0].Connected) // jeœli
                            // jest
                            // coœ
                            // wykryte (a
                            // chyba jest,
                    ...
                    else // if (pVehicles[0]->MoverParameters->DirAbsolute<0) //jeœli
                    // sprzêg 1
                    { // sprzêg 1 - próba podczepienia
                        if (pVehicles[0]->MoverParameters->Couplers[1].Connected) // jeœli
                            // jest
                            // coœ
                            // wykryte (a
                            // chyba jest,
                ...
                /* //Ra 2014-02: lepiej tam, bo jak tam siê odŸwie¿y sk³ad, to tu
                   pVehicles[0]
                   bêdzie czymœ innym
                     else
                     {//jeœli cz³owiek ma pod³¹czyæ, to czekamy na zmianê stanu sprzêgów
                   na koñcach
            ...
            { // jak daleko, to jazda jak dla Shunt na kolizjê
                ...
                if (pVehicles[0]->fTrackBlock <=
                    ...
                    iDrivigFlags |= moveConnect; // pocz¹tek podczepiania, z wy³¹czeniem
                // sprawdzania fTrackBlock
            }
            break;
        case Disconnect: // 20.07.03 - manewrowanie wagonami
            fMinProximityDist = 1.0;
            fMaxProximityDist = 10.0; //[m]
            fVelPlus = 1.0; // dopuszczalne przekroczenie prêdkoœci na ograniczeniu
            // bez hamowania
            fVelMinus = 0.5; // margines prêdkoœci powoduj¹cy za³¹czenie napêdu
            if (AIControllFlag)
            {
                if (iVehicleCount >= 0) // jeœli by³a podana iloœæ wagonów
                {
                    ...
                    { // 2. faza odczepiania: zmieñ
                        // kierunek na przeciwny i dociœnij
                        // za rad¹ yB ustawiamy pozycjê 3 kranu (ruszanie kranem w innych
                        // miejscach
                        // powino zostaæ wy³¹czone)
                        // WriteLog("Zahamowanie sk³adu");
                        // while
                        // ((mvOccupied->BrakeCtrlPos>3)&&mvOccupied->DecBrakeLevel());
                        // while
                        // ((mvOccupied->BrakeCtrlPos<3)&&mvOccupied->IncBrakeLevel());
                        ...
                        if (p < 3.9)
                            p = 3.9; // TODO: zabezpieczenie przed dziwnymi CHK do czasu
                        // wyjaœnienia
                        ...
                        { // jeœli w miarê zosta³ zahamowany
                            // (ciœnienie mniejsze ni¿ podane na
                            ...
                            CheckVehicles(); // od razu zmieniæ œwiat³a (zgasiæ) - bez tego
                            // siê nie
                            // odczepi
        ...
        case Shunt:
            ...
            fVelPlus = 2.0; // dopuszczalne przekroczenie prêdkoœci na ograniczeniu
            // bez hamowania
            fVelMinus = 3.0; // margines prêdkoœci powoduj¹cy za³¹czenie napêdu
            if (fVelMinus > 0.1 * fShuntVelocity)
                fVelMinus = 0.1 * fShuntVelocity; // by³y problemy z jazd¹ np. 3km/h
            // podczas ³adowania wagonów
            break;
        case Obey_train:
            ...
            if (mvOccupied->CategoryFlag & 1) // jeœli poci¹g
            {
                ...
                fMaxProximityDist =
                    (mvOccupied->Vel > 0.0) ? 20.0 : 50.0; //[m] jak stanie za daleko,
                // to niech nie doci¹ga paru
                // metrów
                if (iDrivigFlags & moveLate)
                {
                    fVelMinus = 1.0; // jeœli spóŸniony, to gna
                    fVelPlus = 5.0; // dopuszczalne przekroczenie prêdkoœci na
                    // ograniczeniu bez hamowania
                }
                else
                { // gdy nie musi siê sprê¿aæ
                    ...
                    fVelPlus = int(0.5 + 0.05 * VelDesired); // normalnie dopuszczalne
                    // przekroczenie to 5%
                    // prêdkoœci
                    if (fVelPlus > 5.0)
            ...
            else // samochod (sokista te¿)
            {
                ...
                fMaxProximityDist = 10.0; //[m]
                fVelPlus = 10.0; // dopuszczalne przekroczenie prêdkoœci na ograniczeniu
                // bez hamowania
                fVelMinus = 2.0; // margines prêdkoœci powoduj¹cy za³¹czenie napêdu
            }
            // VelReduced=4; //[km/h]
        ...
        case Prepare_engine: // odpala silnik
            ...
            { // gotowy do drogi?
                SetDriverPsyche();
                // OrderList[OrderPos]:=Shunt; //Ra: to nie mo¿e tak byæ, bo scenerie
                // robi¹
                // Jump_to_first_order i przechodzi w manewrowy
                JumpToNextOrder(); // w nastêpnym jest Shunt albo Obey_train, moze te¿
                // byæ
...
#endif
            break;
        case Wait_for_orders: // jeœli czeka, te¿ ma skanowaæ, ¿eby odpaliæ siê od
        // semafora
        /*
             if ((mvOccupied->ActiveDir!=0))
             {//jeœli jest wybrany kierunek jazdy, mo¿na ustaliæ prêdkoœæ jazdy
              VelDesired=fVelMax; //wstêpnie prêdkoœæ maksymalna dla pojazdu(-ów),
           bêdzie nastêpnie
           ograniczana
              SetDriverPsyche(); //ustawia AccPreferred (potrzebne tu?)
              //Ra: odczyt (ActualProximityDist), (VelNext) i (AccPreferred) z
           tabelki prêdkosci
              AccDesired=AccPreferred; //AccPreferred wynika z osobowoœci mechanika
              VelNext=VelDesired; //maksymalna prêdkoœæ wynikaj¹ca z innych
           czynników ni¿
           trajektoria ruchu
              ActualProximityDist=scanmax; //funkcja Update() mo¿e pozostawiæ
           wartoœci bez zmian
              //hm, kiedyœ semafory wysy³a³y SetVelocity albo ShuntVelocity i
           ustaw³y tak VelSignal
        ...
        */
        ...
        case Change_direction | Connect: // zmiana kierunku podczas pod³¹czania
            ...
            // no albo przypisujemy -WaitingExpireTime, albo porównujemy z
            // WaitingExpireTime
            // if
            ...
            { // automatyczne ruszanie po odstaniu albo spod SBL
                ...
                { // jeœli stoi, a up³yn¹³ czas oczekiwania i tor ma
                    // niezerow¹ prêdkoœæ
                    /*
                         if (WriteLogFlag)
                          {
                            append(AIlogFile);
                            writeln(AILogFile,ElapsedTime:5:2,": ",Name," V=0 waiting time
                       expired!
                       (",WaitingTime:4:1,")");
                            close(AILogFile);
                          }
                    */
                    ...
                    { // samochód ma staæ, a¿ dostanie odjazd, chyba ¿e stoi przez
                        // kolizjê
            ...
            if (AIControllFlag)
                ...
                { // sprobuj zmienic kierunek
                    ...
                    { // jeœli siê zatrzyma³, to zmieniamy
                        // kierunek jazdy, a nawet kabinê/cz³on
                        Activation(); // ustawienie zadanego wczeœniej kierunku i ewentualne
                        // przemieszczenie AI
                        PrepareEngine();
                        JumpToNextOrder(); // nastêpnie robimy, co jest do zrobienia (Shunt
                        // albo
                        // Obey_train)
                        ...
                        /*
                               ...
                               {
                                append(AIlogFile);
                                writeln(AILogFile,ElapsedTime:5:2,": ",Name," Direction
                           changed!");
                                close(AILogFile);
                               }
                        */
            ...
            { // jeœli jest wybrany kierunek jazdy, mo¿na ustaliæ prêdkoœæ jazdy
                // Ra: tu by jeszcze trzeba by³o wstawiæ uzale¿nienie (VelDesired) od
                // odleg³oœci od
                // przeszkody
                ...
                // zmian
                // hm, kiedyœ semafory wysy³a³y SetVelocity albo ShuntVelocity i ustaw³y
                // tak
                ...
                // if (VelSignal!=VelDesired) //je¿eli prêdkoœæ zalecana jest inna (ale
                // tryb te¿
                // mo¿e byæ inny)
                switch (comm)
                { // ustawienie VelSignal - trochê proteza = do przemyœlenia
                case cm_Ready: // W4 zezwoli³ na jazdê
                    TableCheck(scanmax); // ewentualne doskanowanie trasy za W4, który
                    // zezwoli³ na jazdê
                    TableUpdate(VelDesired, ActualProximityDist, VelNext,
                                AccDesired); // aktualizacja po skanowaniu
                    // if (comm!=cm_SetVelocity) //jeœli dalej jest kolejny W4, to ma
                    // zwróciæ
                    // cm_SetVelocity
                    if (VelNext == 0.0)
                        break; // ale jak coœ z przodu zamyka, to ma staæ
                    if (iDrivigFlags & moveStopCloser)
                        VelSignal = VelDesired; // niech jedzie, jak W4 puœci³o - nie, ma czekaæ na
                // sygna³ z sygnalizatora!
                case cm_SetVelocity: // od wersji 357 semafor nie budzi wy³¹czonej
                    // lokomotywy
                    if (!(OrderList[OrderPos] &
                    ...
                    break;
                case cm_ShuntVelocity: // od wersji 357 Tm nie budzi wy³¹czonej
                    // lokomotywy
                    if (!(OrderList[OrderPos] &
                          ~(Obey_train | Shunt))) // jedzie w dowolnym trybie albo Wait_for_orders
                ...
                case cm_Command: // komenda z komórki
                    if (!(OrderList[OrderPos] &
                        ...
                        // iDrivigFlags|=moveStopHere moveStopCloser) //chyba ¿e stan¹³ za
                        // daleko
                ...
                if (VelNext == 0.0)
                    if (!(OrderList[OrderPos] & ~(Shunt | Connect))) // jedzie w Shunt
                    // albo Connect, albo
                    // Wait_for_orders
                    { // je¿eli wolnej drogi nie ma, a jest w trybie manewrowym albo
                        // oczekiwania
                        // if
                        // ((OrderList[OrderPos]&Connect)?pVehicles[0]->fTrackBlock>ActualProximityDist:true)
                        // //pomiar odleg³oœci nie dzia³a dobrze?
                        // w trybie Connect skanowaæ do ty³u tylko jeœli przed kolejnym
                        // sygna³em nie
                        // ma taboru do pod³¹czenia
                        // Ra 2F1H: z tym (fTrackBlock) to nie jest najlepszy pomys³, bo
                        // lepiej by
                        ...
                        if ((OrderList[OrderPos] | Connect) ? pVehicles[0]->fTrackBlock > 2000 :
                            ...
                            { // nale¿y sprawdzaæ odleg³oœæ od znalezionego sygnalizatora,
                                ...
                                if (comm == cm_Command) // jeœli komenda Shunt
                                    iDrivigFlags |= moveStopHere; // to j¹ odbierz bez
                                // przemieszczania siê (np.
                ...
                { // Ra 2F3F: przy jeŸdzie poci¹gowej nie powinien doje¿d¿aæ do
                    // poprzedzaj¹cego
                    // sk³adu
                    if ((mvOccupied->CategoryFlag & 1) ?
                            ((OrderCurrentGet() & (Connect | Obey_train)) == Obey_train) :
                            false) // jeœli jesteœmy poci¹giem a jazda poci¹gowa i nie
                    // œci¹ganie ze
                    // szlaku
                    {
                        pVehicles[0]->ABuScanObjects(pVehicles[0]->DirectionGet(),
                                                     1000.0); // skanowanie sprawdzaj¹ce
                        // Ra 2F3F: i jest problem, jak droga za semaforem kieruje na jakiœ
                        // pojazd
                        ...
                        if (pVehicles[0]->fTrackBlock < 1000.0) // i jeœli nadal coœ jest
                            if (VelNext != 0.0) // a nastêpny sygna³ zezwala na jazdê
                                if (pVehicles[0]->fTrackBlock <
                                    ActualProximityDist) // i jest bli¿ej (tu by trzeba by³o
                                    // wstawiæ
                ...
                // if (mvOccupied->Vel>=0.1) //o ile jedziemy; jak stoimy to te¿ trzeba
                // jakoœ
                // zatrzymywaæ
                ...
                { // sprawdzenie jazdy na widocznoœæ
                    ...
                    if (c->Connected) // a mamy coœ z przodu
                        ...
                        { // wyliczanie optymalnego przyspieszenia do jazdy na widocznoœæ
                            ...
                            { // zatroszczyæ siê trzeba, jeœli tamten nie jedzie znacz¹co
                                // szybciej
                                double d = pVehicles[0]->fTrackBlock - 0.5 * vel -
                                           fMaxProximityDist; // odleg³oœæ bezpieczna zale¿y od
                                // prêdkoœci
                                if (d < 0) // jeœli odleg³oœæ jest zbyt ma³a
                                { // AccPreferred=-0.9; //hamowanie maksymalne, bo jest za
                                    // blisko
                                    ...
                                    else // jeœli oba jad¹, to przyhamuj lekko i ogranicz
                                    // prêdkoœæ
                                    {
                                ...
                                { // jeœli odleg³oœæ jest wiêksza, ustaliæ maksymalne
                                    // mo¿liwe
                                    ...
                                    if (d < fBrakeDist) // a jest w drodze hamowania
                                        if (k < AccPreferred)
                                        { // jeœli nie ma innych powodów do
                                            // wolniejszej jazdy
                                            AccPreferred = k;
                ...
                if (TrainParams)
                    if (TrainParams->CheckTrainLatency() < 5.0)
                        if (TrainParams->TTVmax > 0.0)
                            VelDesired = Min0R(VelDesired,
                                               TrainParams->TTVmax); // jesli nie spozniony to
                // nie przekraczaæ
                // rozkladowej
                if (VelDesired > 0.0)
                    if (VelNext > 0.0)
                    { // jeœli mo¿na jechaæ, to odpaliæ dŸwiêk
                        // kierownika oraz zamkn¹æ drzwi w
                        // sk³adzie
                        if (iDrivigFlags & moveGuardSignal)
                        { // komunikat od kierownika tu,
                            // bo musi byæ wolna droga i
                            // odczekany czas
                            // stania
                            iDrivigFlags &= ~moveGuardSignal; // tylko raz nadaæ
                            tsGuardSignal->Stop();
                            // w zasadzie to powinien mieæ flagê, czy jest dŸwiêkiem radiowym,
                            // czy
                            // bezpoœrednim
                            // albo trzeba zrobiæ dwa dŸwiêki, jeden bezpoœredni, s³yszalny w
                            // pobli¿u, a drugi radiowy, s³yszalny w innych lokomotywach
                            // na razie zak³adam, ¿e to nie jest dŸwiêk radiowy, bo trzeba by
                            // zrobiæ
                            ...
                            if (!iGuardRadio) // jeœli nie przez radio
                                ...
                                // if (!FreeFlyModeFlag) //obserwator musi byæ w œrodku
                                // pojazdu
                                // (albo mo¿e mieæ radio przenoœne) - kierownik móg³by
                                // powtarzaæ
                                // przy braku reakcji
                        ...
                        if (iDrivigFlags & moveDoorOpened) // jeœli drzwi otwarte
                            if (!mvOccupied->DoorOpenCtrl) // jeœli drzwi niesterowane przez
                                // maszynistê
...
// AbsAccS=-AbsAccS; //to przyspieszenie te¿ dzia³a wtedy w nieodpowiedni¹
// stronê
// AbsAccS+=fAccGravity; //wypadkowe przyspieszenie (czy to ma sens?)
                ...
                //(AccPreferred) - wynika z psychyki oraz uwzglênia ju¿ ewentualne
                // zderzenie z
                // pojazdem z przodu, ujemne gdy nale¿y hamowaæ
                //(AccDesired) - uwzglêdnia sygna³y na drodze ruchu, ujemne gdy nale¿y
                // hamowaæ
                //(fAccGravity) - chwilowe przspieszenie grawitacyjne, ujemne dzia³a
                // przeciwnie do
                // zadanego kierunku jazdy
                //(AbsAccS) - chwilowe przyspieszenie pojazu (uwzglêdnia grawitacjê),
                // ujemne dzia³a
                // przeciwnie do zadanego kierunku jazdy
                //(AccDesired) porównujemy z (fAccGravity) albo (AbsAccS)
                // if
                // ((VelNext>=0.0)&&(ActualProximityDist>=0)&&(mvOccupied->Vel>=VelNext))
                // //gdy
                // zbliza sie i jest za szybko do NOWEGO
                if ((VelNext >= 0.0) && (ActualProximityDist <= scanmax) && (vel >= VelNext))
                { // gdy zbli¿a siê i jest za szybki do nowej
                    // prêdkoœci, albo stoi na zatrzymaniu
                    if (vel > 0.0)
                    { // jeœli jedzie
                        if ((vel < VelNext) ?
                                (ActualProximityDist > fMaxProximityDist * (1 + 0.1 * vel)) :
                                false) // dojedz do semafora/przeszkody
                        { // jeœli jedzie wolniej ni¿ mo¿na i jest wystarczaj¹co daleko, to
                            // mo¿na
                            // przyspieszyæ
                        ...
                        { // jedzie szybciej, ni¿ trzeba na
                            // koñcu ActualProximityDist, ale
                            // jeszcze
                            // jest daleko
                            if (vel <
                            ...
                            { // jeœli jedzie wolniej ni¿ VelNext+35km/h //Ra: 40, ¿eby nie
                                ...
                                if (VelNext == 0.0)
                                { // jeœli ma siê zatrzymaæ, musi byæ to
                                    // robione precyzyjnie i
                                    // skutecznie
                                    ...
                                    { // jeœli ma stan¹æ, a mieœci siê w
                                        // drodze hamowania
                                ...
                                { // jeœli wiêksze przekroczenie ni¿ fVelPlus [km/h], ale
                                    // ma jechaæ
                            ...
                            else // przy du¿ej ró¿nicy wysoki stopieñ (1,25 potrzebnego
                                // opoznienia)
                                AccDesired = (VelNext * VelNext - vel * vel) /
                            ...
                            // //aby szybkosc hamowania zalezala od przyspieszenia i
                            // opoznienia
                            // hamulcow
                ...
                else // gdy jedzie wolniej ni¿ potrzeba, albo nie ma przeszkód na
                    // drodze
                ...
                { // Ra 2014-03: to nie uwzglêdnia odleg³oœci i zaczyna hamowaæ, jak
                    // tylko zobaczy
                    // W4
                    if ((AccDesired > 0.0) &&
                ...
                if (AIControllFlag)
                { // czêœæ wykonawcza tylko dla AI, dla cz³owieka
                    // jedynie napisy
                    if (mvControlling->ConvOvldFlag ||
                    ...
                    // margines dla prêdkoœci jest doliczany tylko jeœli oczekiwana
                    // prêdkoœæ jest
                    // wiêksza od 5km/h
                    if (!(iDrivigFlags & movePress))
                    { // jeœli nie dociskanie
                        if (AccDesired < -0.1)
                            ...
                            // przekroczona
                            DecSpeed(); // pojedyncze cofniêcie pozycji, bo na zero to
                        // przesada
                    }
                    // yB: usuniête ró¿ne dziwne warunki, oddzielamy czêœæ zadaj¹c¹ od
                    // wykonawczej
                    // zwiekszanie predkosci
                    // Ra 2F1H: jest konflikt histerezy pomiêdzy nastawion¹ pozycj¹ a
                    // uzyskiwanym
                    // przyspieszeniem - utrzymanie pozycji powoduje przekroczenie
                    // przyspieszenia
                    if (AbsAccS < AccDesired) // jeœli przyspieszenie pojazdu jest
                        // mniejsze ni¿ ¿¹dane oraz
                    ...
                    { // Ra: to dobrze hamuje EP w EZT
                        if ((AccDesired <= fAccThreshold) ? // jeœli hamowaæ - u góry ustawia siê
                                ...
                                false) // hamowaæ bardziej, gdy aktualne opóŸnienie
                            // hamowania
                            // mniejsze ni¿ (AccDesired)
                            IncBrake();
                        else if (OrderList[OrderPos] !=
                                 Disconnect) // przy od³¹czaniu nie zwalniamy tu hamulca
                            if (AbsAccS < AccDesired - 0.05) // jeœli opóŸnienie wiêksze od
                            // wymaganego (z histerez¹)
                        ...
                        //         else if (mvOccupied->BrakeCtrlPos<0) IncBrake();
                        //         //ustawienie
                        //         jazdy (pozycja 0)
                    ...
                    { // a stara wersja w miarê dobrze dzia³a na sk³ady wagonowe
                        ...
                        // if ((AccDesired<0.0)&&(AbsAccS<AccDesired-0.1)) //ST44 nie hamuje
                        // na
                    ...
                    /* //Ra: mamy teraz wska¿nik na cz³on silnikowy, gorzej jak s¹ dwa w
                       ukrotnieniu...
                          //zapobieganie poslizgowi w czlonie silnikowym; Ra: Couplers[1]
                       powinno
                       byæ
                          if (Controlling->Couplers[0].Connected!=NULL)
                           if
                       (TestFlag(Controlling->Couplers[0].CouplingFlag,ctrain_controll))
                            if (Controlling->Couplers[0].Connected->SlippingWheels)
                             if
                       (Controlling->ScndCtrlPos>0?!Controlling->DecScndCtrl(1):true)
                             {
                ...
                { // tu mozna daæ komunikaty tekstowe albo s³owne: przyspiesz,
                    // hamuj (lekko,
                    // œrednio, mocno)
                }
            ...
            if (AIControllFlag)
            { // odhamowywanie sk³adu po zatrzymaniu i
                // zabezpieczanie lokomotywy
    ...
    if (AIControllFlag)
    {
        ...
        { // tr¹bienie trwa nadal
            ...
            if (ReactionTime > fWarningDuration)
                ReactionTime = fWarningDuration; // wczeœniejszy przeb³ysk œwiadomoœci,
            // by zakoñczyæ tr¹bienie
        }
        if (mvOccupied->Vel >= 3.0) // jesli jedzie, mo¿na odblokowaæ tr¹bienie, bo
        // siê wtedy nie w³¹czy
        {
...
{ // wykonanie kolejnej komendy z tablicy
    // rozkazów
    if (OrderList[OrderPos] != Wait_for_orders)
    {
...
{ // zapisanie na stosie kolejnego rozkazu do wykonania
    if (OrderPos == OrderTop) // jeœli mia³by byæ zapis na aktalnej pozycji
        if (OrderList[OrderPos] < Shunt) // ale nie jedzie
            ++OrderTop; // niektóre operacje musz¹ zostaæ najpierw dokoñczone => zapis
    // na kolejnej
...
{ // wype³nianie tabelki rozkazów na podstawie rozk³adu
    ...
    { // brak rozk³adu to jazda manewrowa
        if (fVel > 0.05) // typowo 0.1 oznacza gotowoœæ do jazdy, 0.01 tylko
            // za³¹czenie silnika
        ...
        if ((fVel > 0.0) && (fVel < 0.02))
            OrderPush(Shunt); // dla prêdkoœci 0.01 w³¹czamy jazdê manewrow¹
                     ...
                     (TrainParams->TimeTable[1].StationWare.Pos(
                          ...
                          false) :
                     true)
            OrderPush(Shunt); // a teraz start bêdzie w manewrowym, a tryb poci¹gowy
        // w³¹czy W4
        else
            // jeœli start z pierwszej stacji i jednoczeœnie jest na niej zmiana
            // kierunku, to EZT ma
            // mieæ Shunt
    ...
    { // jeœli podana niezerowa prêdkoœæ
        if ((fVel >= 1.0) || (fVel < 0.02)) // jeœli ma jechaæ - dla 0.01 ma
            // podjechaæ manewrowo po podaniu
            // sygna³u
...
// Ra: stare funkcje skanuj¹ce, u¿ywane podczas manewrów do szukania
// sygnalizatora z ty³u
//- nie reaguj¹ na PutValues, bo nie ma takiej potrzeby
//- rozpoznaj¹ tylko zerow¹ prêdkoœæ (jako koniec toru i brak podstaw do
// dalszego skanowania)
//----------------------------------------------------------------------------------------------------------------------

/* //nie u¿ywane
double TController::Distance(vector3 &p1,vector3 &n,vector3 &p2)
{//Ra:obliczenie odleg³oœci punktu (p1) od p³aszczyzny o wektorze normalnym (n)
przechodz¹cej przez
(p2)
 return n.x*(p1.x-p2.x)+n.y*(p1.y-p2.y)+n.z*(p1.z-p2.z); //ax1+by1+cz1+d, gdzie
d=-(ax2+by2+cz2)
};
*/
...
{ // sprawdzanie eventu w torze, czy jest
    // sygna³owym - skanowanie do ty³u
    TEvent *e = (fDirection > 0) ? Track->evEvent2 : Track->evEvent1;
    if (e)
...
{ // szukanie sygnalizatora w kierunku przeciwnym jazdy
    // (eventu odczytu komórki pamiêci)
    ...
    while (s < fDistance)
    {
        ...
        s += fCurrentDistance; // doliczenie kolejnego odcinka do przeskanowanej
        // d³ugoœci
        if (fDirection > 0)
        { // jeœli szukanie od Point1 w kierunku Point2
        ...
        if (Track ?
                ...
                true)
        { // gdy dalej toru nie ma albo zerowa prêdkoœæ, albo
            // uszkadza pojazd
            fDistance = s;
...
{ // Ra:przeslanie do AI prêdkoœci
    /*
     ...
     if
     ((vel<0)?true:dist>0.1*(MoverParameters->Vel*MoverParameters->Vel-vel*vel)+50)
...
{ // sprawdzanie zdarzeñ semaforów z
    // ty³u pojazdu, zwraca komendê
    // dziêki temu bêdzie mo¿na stawaæ za wskazanym sygnalizatorem, a zw³aszcza
    // jeœli bêdzie jazda
    // na kozio³
    // ograniczenia prêdkoœci nie s¹ wtedy istotne, równie¿ koniec toru jest do
    // niczego nie
    // przydatny
    // zwraca true, jeœli nale¿y odwróciæ kierunek jazdy pojazdu
    if ((OrderList[OrderPos] & ~(Shunt | Connect)))
        return cm_Unknown; // skanowanie sygna³ów tylko gdy jedzie w trybie
    // manewrowym albo czeka na
    // rozkazy
    vector3 sl;
    int startdir = -pVehicles[0]->DirectionGet(); // kierunek jazdy wzglêdem
    // sprzêgów pojazdu na czele
    ...
    if (scandir != 0.0) // skanowanie toru w poszukiwaniu eventów GetValues
    // (PutValues nie s¹ przydatne)
    { // Ra: przy wstecznym skanowaniu prêdkoœæ nie ma znaczenia
        // scanback=pVehicles[1]->NextDistance(fLength+1000.0); //odleg³oœæ do
        // nastêpnego pojazdu,
        // 1000 gdy nic nie ma
        ...
        // opcjonalnie mo¿e byæ skanowanie od "wskaŸnika" z przodu, np. W5, Tm=Ms1,
        // koniec toru
        ...
        { // a jeœli s¹ dalej tory
            ...
            { // jeœli jest jakiœ sygna³ na widoku
                ...
                { // przes³aæ info o zbli¿aj¹cym siê semaforze
                    ...
                    // sem=e->Params[8].asGroundNode->pCenter-pos; //wektor do komórki
                    // pamiêci
                    if (dir.x * sem.x + dir.z * sem.z < 0) // jeœli zosta³ miniêty
                    // if ((mvOccupied->CategoryFlag&1)?(VelNext!=0.0):true) //dla poci¹gu
                    // wymagany
                    // sygna³ zezwalaj¹cy
                    ...
                    // przeliczamy odleg³oœæ od semafora - potrzebne by by³y wspó³rzêdne
                    // pocz¹tku
                    // sk³adu
                    ...
                    if (e->Command() == cm_SetVelocity)
                        ...
                        { //
                            if ((scandist > fMinProximityDist) ?
                                    ...
                                    false)
                            { // jeœli semafor jest daleko, a pojazd jedzie,
// to informujemy o
// zmianie prêdkoœci
// jeœli jedzie manewrowo, musi dostaæ SetVelocity, ¿eby sie na poci¹gowy
// prze³¹czy³
// Mechanik->PutCommand("SetProximityVelocity",scandist,vmechmax,sl);
                            ...
                            else // ustawiamy prêdkoœæ tylko wtedy, gdy ma ruszyæ, stan¹æ
                            // albo ma
                            // staæ
                            // if ((MoverParameters->Vel==0.0)||(vmechmax==0.0)) //jeœli stoi
                            // lub ma
                            // stan¹æ/staæ
                            { // semafor na tym torze albo lokomtywa stoi, a ma ruszyæ, albo
// ma
                            ...
                            { // jeœli tarcza jest daleko, to:
                                ...
                                { // Mechanik->PutCommand("SetProximityVelocity",scandist,vmechmax,sl);
                                    ...
                                    return (iDrivigFlags & moveTrackEnd) ?
                                               cm_ChangeDirection :
                                               cm_Unknown; // jeœli jedzie na W5 albo koniec
                                    // toru,
                                    // to mo¿na zmieniæ kierunek
                                }
                            }
                            else // ustawiamy prêdkoœæ tylko wtedy, gdy ma ruszyæ, albo
                            // stan¹æ albo
                            // ma staæ pod tarcz¹
                            { // stop trzeba powtarzaæ, bo inaczej zatr¹bi i pojedzie sam
                                // if ((MoverParameters->Vel==0.0)||(vmechmax==0.0)) //jeœli
                                // jedzie
                                // lub ma stan¹æ/staæ
...
// PutCommand("ShuntVelocity",vmechmax,e->Params[9].asMemCell->Value2(),&sl,stopSem);
                            ...
                            { // jeœli Tm w odleg³oœci do 100m podaje
// zezwolenie na jazdê, to od
...
{ // przejêcie kontroli przez AI albo oddanie
    ...
    { // teraz AI prowadzi
        ...
        // gdy zgaszone œwiat³a, flaga podje¿d¿ania pod semafory pozostaje bez
        // zmiany
        ...
        */
        ...
        TableClear(); // ponowne utworzenie tabelki, bo cz³owiek móg³ pojechaæ
        // niezgodnie z
        // sygna³ami
    }
    else
...
{ // ustawienie jazdy do przodu
    // dla true i do ty³u dla
    // false (zale¿y od kabiny)
    while (mvControlling->MainCtrlPos) // samo zapêtlenie DecSpeed() nie wystarcza
        DecSpeed(true); // wymuszenie zerowania nastawnika jazdy, inaczej siê mo¿e
    // zawiesiæ
    if (forward)
        while (mvOccupied->ActiveDir <= 0)
...
{ // zwraca indeks aktualnej stacji (miejsca
    // zatrzymania)
    return TrainParams->StationIndex;
};

bool TController::IsStop()
{ // informuje, czy jest zatrzymanie na najbli¿szej
    // stacji
    return TrainParams->IsStop();
};

void TController::MoveTo(TDynamicObject *to)
{ // przesuniêcie AI do innego
    // pojazdu (przy zmianie kabiny)
...
{ // znajduje cz³on do sterowania w EZT
    // bêdzie to silnikowy
...
{ // pozycja tabelki prêdkoœci
    ...
    if (i != iLast) // w (iLast) znajduje siê kolejny tor do przeskanowania, ale
        // nie jest ona
        // aktywn¹
...
{ // zwraca numer segmentu dla skrzy¿owania (tr)
    ...
    // droga powinna byæ okreœlona sposobem przejazdu przez skrzy¿owania albo
    // wspó³rzêdnymi miejsca
    // docelowego
    for (int i = iFirst; i != iLast; i = (i + 1) % iSpeedTableSize)
    { // trzeba przejrzeæ tabelê skanowania w
        // poszukiwaniu (tr)
        // i jak siê znajdzie, to zwróciæ zapamiêtany numer segmentu i kierunek
        // przejazdu
        // (-6..-1,1..6)
